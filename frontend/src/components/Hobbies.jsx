...
function HobbyCard({ hobby, icon, iconBg, index }) {
  ...
  return (
    <div
      ...
      className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 transition-all duration-500 hover:border-teal-500 ${
    ...
    >
      ...
      <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-3">{hobby.title}</h3>
      <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-5">{hobby.description}</p>
      <div className="flex flex-wrap gap-2 mb-4">
        {hobby.tags.map((tag) => (
          <span
            key={tag}
            className="px-3 py-1 text-xs font-medium text-teal-700 dark:text-teal-300 border border-teal-300 dark:border-teal-700 bg-teal-50 dark:bg-teal-950/50 rounded-full"
          >
            {tag}
          </span>
        ))}
